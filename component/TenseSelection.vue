<template>
    <div class="w-full max-w-4xl mx-auto my-6 px-4 space-y-4">
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-2 p-1.5 bg-slate-800/50 backdrop-blur-md rounded-2xl border border-white/5 shadow-inner">
                <button v-for="parent in ['Present', 'Past', 'Future']" :key="parent"
                    :class="[
                        'relative px-5 py-3 rounded-xl text-sm font-bold transition-all duration-300 ease-out flex-1 sm:flex-none whitespace-nowrap overflow-hidden',
                    ...
                    <span class="relative z-10">{{ parent }} Tense</span>
                </button>
            </div>
        </div>
        ...
        <div v-if="selectedParent && getChildTenses(selectedParent).length > 0">
            ...
            <div class="flex flex-wrap gap-2 p-1.5 bg-slate-800/30 backdrop-blur-sm rounded-xl border border-white/5">
                <button 
                    :class="[
                        'px-4 py-2 rounded-lg text-xs font-medium transition-all duration-200 ease-out',
                        !selectedChild 
                            ? 'bg-purple-500/20 text-purple-300 border border-purple-500/30 shadow-sm' 
                            : 'text-slate-400 hover:text-slate-200 hover:bg-slate-700/50 border border-transparent'
                    ]"
                    @click="$emit('select-child', null)">
                    All {{ selectedParent }} Types
                </button>
